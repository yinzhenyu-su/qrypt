...
func NewBundleCmd(rt cliruntime.Runtime) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "bundle [REMOTE]",
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if outPath == "" {
				return rt.UsageError(cmd, "missing --out FILE")
			}
			...
			if watchDuration > 0 {
				if err := cliruntime.ValidateSamplingWindow(watchDuration, watchInterval, "watch", "watch-interval"); err != nil {
					return err
				}
			}
			...
			err = util.WriteAtomic(outPath, ".qrypt-debug-*.zip", 0o600, force, func(out *os.File) error {
