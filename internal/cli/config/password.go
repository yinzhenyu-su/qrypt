...
func NewExportRclonePasswordCmd(rt cliruntime.Runtime) *cobra.Command {
	cmd := &cobra.Command{
...
Or compute directly from raw inputs (no config file needed):
  qrypt config export-rclone-password --password-file ./password.txt --salt "mysalt"`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if state.Cfg == nil {
				return fmt.Errorf("%w; alternatively use --password-file or --password-stdin", rt.ConfigNotFoundError())
			}
...
func DirectPasswordFromFlags(cmd *cobra.Command) (string, bool, error) {
	...
	if passwordFile != "" {
		raw, err := os.ReadFile(util.ExpandHome(passwordFile))
		if err != nil {
