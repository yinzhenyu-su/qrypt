...
// for a retry. waitIdle drains async VFS work between passes. Returns the
// number of failed items.
func ExecutePlan(ctx context.Context, fs vfs.FileSystem, plan []PlanEntry, source, destination Target, session *Session, waitIdle func(context.Context, vfs.FileSystem) error) int {
	failed := 0
...
func DeletePath(ctx context.Context, fs vfs.FileSystem, rel string, destination Target, isDir bool) error {
...
func Mkdir(ctx context.Context, fs vfs.FileSystem, rel string, destination Target) error {
	switch destination.Kind {
	case TargetVFS:
...
func CopyFile(ctx context.Context, fs vfs.FileSystem, rel string, source, destination Target, sourceModTime int64) error {
...
// converge).
func SetModTime(ctx context.Context, fs vfs.FileSystem, path string, modTime int64) error {
	if modTime <= 0 {
		return nil
	}
