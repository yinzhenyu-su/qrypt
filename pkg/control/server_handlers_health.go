...
func (s *Server) handleOps(w http.ResponseWriter, r *http.Request) {
	...
	provider, ok := s.source.(diagnostics.DebugActiveProvider)
	if !ok {
...
func transferMountContext(snapshot diagnostics.DebugSnapshot, mountName string) TransferMountContext {
...
func (s *Server) handleConsistency(w http.ResponseWriter, r *http.Request) {
	...
	checker, ok := s.source.(diagnostics.DebugConsistencyChecker)
	if !ok {
...
func (s *Server) consistencyReports(ctx context.Context, checker diagnostics.DebugConsistencyChecker, dir string, recursive bool) ([]diagnostics.ConsistencyReport, error) {
	...
	sort.Strings(all)
	reports := make([]diagnostics.ConsistencyReport, 0, len(all))
...
func (s *Server) handleHealth(w http.ResponseWriter, r *http.Request) {
	...
	taskPersistence := TaskPersistenceStatus{}
	if s.taskHealth != nil {
		if err := s.taskHealth.TaskPersistenceError(); err != nil {
