...
func TestWriteAtomicWithOptionsCreatesParentAndSyncs(t *testing.T) {
	path := filepath.Join(t.TempDir(), "nested", "output")
	err := WriteAtomicWithOptions(path, AtomicWriteOptions{
		Pattern:      ".output-*",
		Mode:         0o600,
		Replace:      true,
		CreateParent: true,
		...
		_, err := file.WriteString("durable")
		return err
	})
	if err != nil {
		t.Fatalf("WriteAtomicWithOptions() error = %v", err)
	}
	data, err := os.ReadFile(path)
	if err != nil {
		t.Fatal(err)
	}
	if got := string(data); got != "durable" {
		t.Fatalf("content = %q, want durable", got)
	}
}

func TestWriteAtomicWithOptionsRejectsInvalidInput(t *testing.T) {
	if err := WriteAtomicWithOptions("", AtomicWriteOptions{}, func(*os.File) error { return nil }); err == nil {
		t.Fatal("empty destination accepted")
	}
	if err := WriteAtomicWithOptions(filepath.Join(t.TempDir(), "output"), AtomicWriteOptions{}, nil); err == nil {
