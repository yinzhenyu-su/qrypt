...
func TestFormatUnixNano(t *testing.T) {
	if got := FormatUnixNano(0); got != "-" {
		t.Fatalf("FormatUnixNano(0) = %q, want -", got)
	}
	ts := time.Date(2026, 8, 12, 9, 10, 11, 123, time.UTC)
	want := time.Unix(0, ts.UnixNano()).Format(time.RFC3339)
	if got := FormatUnixNano(ts.UnixNano()); got != want {
		t.Fatalf("FormatUnixNano() = %q, want %q", got, want)
	}
}
