...
func (r vfsVisibilityRuntime) RestoreDeletedPath(path string) (drive.Entry, bool) {
	...
	delete(r.v.deleteTasks.failures, path)
	if timer := r.v.deleteTasks.timers[path]; timer != nil {
		timer.Stop()
		delete(r.v.deleteTasks.timers, path)
...
func (r vfsVisibilityRuntime) RestoreDeletedAncestor(path string) {
	...
	delete(r.v.deleteTasks.failures, restorePath)
	if timer := r.v.deleteTasks.timers[restorePath]; timer != nil {
		timer.Stop()
		delete(r.v.deleteTasks.timers, restorePath)
