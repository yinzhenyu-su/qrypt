...
func newVFSDebugUploadRuntime(svc *uploadService) vfsDebugUploadRuntime {
	return vfsDebugUploadRuntime{svc: svc}
}

func (r vfsDebugUploadRuntime) ActiveSnapshots() map[string]uploadSnapshot {
	active := map[string]uploadSnapshot{}
	r.svc.DebugState().Mu.Lock()
	for path, state := range r.svc.DebugState().Active {
		active[path] = cloneUploadSnapshot(state.Upload)
	}
	r.svc.DebugState().Mu.Unlock()
	return active
}

func (r vfsDebugUploadRuntime) History() []uploadSnapshot {
	r.svc.DebugState().Mu.Lock()
	defer r.svc.DebugState().Mu.Unlock()
	out := make([]uploadSnapshot, len(r.svc.DebugState().History))
	for i, upload := range r.svc.DebugState().History {
...
func (r vfsDebugUploadRuntime) RemoveHistoryByID(id string) bool {
	...
	for i, upload := range r.svc.DebugState().History {
		...
		copy(r.svc.DebugState().History[i:], r.svc.DebugState().History[i+1:])
		r.svc.DebugState().History = r.svc.DebugState().History[:len(r.svc.DebugState().History)-1]
		return true
	}
	return false
}
...
func (r vfsDebugUploadRuntime) StartSnapshot(p PendingUpload) {
	now := util.Now()
	r.svc.DebugState().Mu.Lock()
	r.svc.DebugState().Active[p.Path] = &uploadSnapshotState{
		StageStartedAt: now,
...
func (r vfsDebugUploadRuntime) SetSnapshotState(path, state string) {
	r.svc.DebugState().Mu.Lock()
	if upload := r.svc.DebugState().Active[path]; upload != nil {
...
func (r vfsDebugUploadRuntime) FinishSnapshot(path, state, lastError string) {
	...
	if upload := r.svc.DebugState().Active[path]; upload != nil {
		...
		r.svc.DebugState().History = append(r.svc.DebugState().History, upload.Upload)
		if len(r.svc.DebugState().History) > uploadSnapshotHistoryLimit {
			copy(r.svc.DebugState().History, r.svc.DebugState().History[len(r.svc.DebugState().History)-uploadSnapshotHistoryLimit:])
			r.svc.DebugState().History = r.svc.DebugState().History[:uploadSnapshotHistoryLimit]
		}
		delete(r.svc.DebugState().Active, path)
	}
	r.svc.DebugState().Mu.Unlock()
}
...
func (r vfsDebugUploadRuntime) SetSnapshotMetadata(path, resultRemoteID string, hashes []string) {
	r.svc.DebugState().Mu.Lock()
	if state := r.svc.DebugState().Active[path]; state != nil {
		if resultRemoteID != "" {
