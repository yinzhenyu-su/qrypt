...
func TestVFSDeleteTaskRuntimeBuildsRecordsFromDeleteState(t *testing.T) {
	...
	fs.delete.tasks.timers["/scheduled.txt"] = time.AfterFunc(time.Hour, func() {})
	fs.delete.tasks.active["/running.txt"] = drive.Entry{ID: "running", Name: "running.txt"}
	fs.delete.tasks.failures["/failed.txt"] = "remote failed"
...
func TestVFSDeleteTaskRuntimeRetryClearsFailureAndSchedulesDelete(t *testing.T) {
	...
	fs.view.overlay.mu.Lock()
	fs.delete.tasks.failures[record.path] = "remote failed"
	fs.view.overlay.mu.Unlock()
	defer newVFSDeleteScheduler(fs).StopAll()

	runtime.Retry(record)
	fs.view.overlay.mu.Lock()
	_, failure := fs.delete.tasks.failures[record.path]
	_, timer := fs.delete.tasks.timers[record.path]
	fs.view.overlay.mu.Unlock()
	if failure {
