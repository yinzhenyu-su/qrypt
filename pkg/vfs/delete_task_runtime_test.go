...
func TestVFSDeleteTaskRuntimeBuildsRecordsFromDeleteState(t *testing.T) {
	...
	fs.deleteTasks.timers["/scheduled.txt"] = time.AfterFunc(time.Hour, func() {})
	fs.deleteTasks.active["/running.txt"] = drive.Entry{ID: "running", Name: "running.txt"}
	fs.deleteTasks.failures["/failed.txt"] = "remote failed"
...
func TestVFSDeleteTaskRuntimeRetryClearsFailureAndSchedulesDelete(t *testing.T) {
	...
	fs.view.overlay.mu.Lock()
	fs.deleteTasks.failures[record.path] = "remote failed"
	fs.view.overlay.mu.Unlock()
	defer newVFSDeleteScheduler(fs).StopAll()

	runtime.Retry(record)
	fs.view.overlay.mu.Lock()
	_, failure := fs.deleteTasks.failures[record.path]
	_, timer := fs.deleteTasks.timers[record.path]
	fs.view.overlay.mu.Unlock()
	if failure {
