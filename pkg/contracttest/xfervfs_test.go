...
func (f *xferFakeFS) DebugSnapshot() diagnostics.DebugSnapshot {
	snapshot := diagnostics.DebugSnapshot{
		SchemaVersion: diagnostics.DebugSnapshotSchemaVersion,
		Kind:          "namespace",
		Mounts: []diagnostics.MountSnapshot{
			{Identity: diagnostics.MountSnapshotIdentity{Name: "src", DriverName: "source-test"}},
			{Identity: diagnostics.MountSnapshotIdentity{Name: "dst", DriverName: "dest-test"}},
		},
	}
	...
	for path, data := range f.files {
		for i := range snapshot.Mounts {
			...
			snapshot.Mounts[i].UploadState.History = append(snapshot.Mounts[i].UploadState.History, upload.UploadSnapshot{
				Path:          localPath,
				State:         "completed",
