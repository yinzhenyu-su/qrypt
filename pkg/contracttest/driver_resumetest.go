...
func RunVFSResumeTest(ctx context.Context, fs vfs.FileSystem, mount string, size int64) *ResumeTestResult {
	...
	start = time.Now()
	injector, ok := fs.(faultinject.DebugUploadCancelInjector)
	if !ok {
		err = fmt.Errorf("VFS resume test not available: filesystem does not support debug upload cancel injection")
	} else {
		var injected faultinject.DebugUploadCancelResult
		afterBytes := minInt64(size/4, 16<<20)
		if afterBytes < 1 {
			afterBytes = 1
		}
		injected, err = injector.DebugInjectUploadCancel(ctx, faultinject.DebugUploadCancelRequest{
			Path:       file,
