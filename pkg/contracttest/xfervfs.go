...
func findVFSUpload(snapshot diagnostics.DebugSnapshot, path string) (upload.UploadSnapshot, string, string, bool) {
	for _, mount := range snapshot.Mounts {
		localPath := path
	...
	return upload.UploadSnapshot{}, "", "", false
}

func findUploadSnapshot(uploads []upload.UploadSnapshot, path string) (upload.UploadSnapshot, bool) {
	for i := len(uploads) - 1; i >= 0; i-- {
		if vfs.CleanVirtualPath(uploads[i].Path) == vfs.CleanVirtualPath(path) {
