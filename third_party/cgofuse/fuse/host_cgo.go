...
#if defined(__APPLE__)
	if (capNodeRWLock)
		conn->want |= conn->capable & FUSE_CAP_NODE_RWLOCK;
...
func c_hostAsgnCconninfo(conn *c_struct_fuse_conn_info,
	...
	capNodeRWLock c_bool,
	capReaddirPlus c_bool,
	capDeleteAccess c_bool,
	capOpenTrunc c_bool) {
	C.hostAsgnCconninfo(conn, capCaseInsensitive, capNodeRWLock, capReaddirPlus, capDeleteAccess, capOpenTrunc)
}
